...
def main():
    ...
    publishers = get_field(papers, "publisher")

    publisher_counts = Counter(publishers)
    
    visualize(publisher_counts)
...
def visualize(counts: Counter) -> None:
    ...
    p = figure(title="publishers of AutoML for EO papers", x_axis_label="publisher", y_axis_label="amount", x_range=labels)
